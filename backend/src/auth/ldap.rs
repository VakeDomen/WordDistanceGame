...
pub async fn stdent_ldap_login(username: String, password: String) -> Result<Option<String>> {
    let (conn, ldap_conn) =
        LdapConnAsync::new(&env::var("LDAP_SERVER_STUDENT").expect("$LDAP_SERVER is not set"))
            .await?;
...
pub async fn employee_ldap_login(username: String, password: String) -> Result<Option<String>> {
    ...
    let (conn, ldap_conn) =
        LdapConnAsync::new(&env::var("LDAP_SERVER_EMPLOYEE").expect("$LDAP_SERVER is not set"))
            .await?;
